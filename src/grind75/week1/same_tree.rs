...
mod tests {
    ...
    #[test]
    fn equal_cases() {
        let tree1 = TreeNode::from_bfs(vec![Some(2), Some(3), None, Some(1)]);
        ...
        let tree1 = TreeNode::from_bfs(vec![]);
    ...
    fn different_values() {
        ...
        let tree1 = TreeNode::from_bfs(vec![Some(0), Some(-5)]);
        let tree2 = TreeNode::from_bfs(vec![Some(0), Some(-8)]);
        assert_eq!(is_same_tree(tree1.clone(), tree2.clone()), false);
        assert_eq!(is_same_tree_builtin(tree1.clone(), tree2.clone()), false);
        assert_eq!(is_same_tree_iterative(tree1.clone(), tree2.clone()), false);
    }
