        ...
        'outer_while_loop: loop {
            ...
            for _ in 0..node_in_prev_level {
                ...
                if let Some(value) = v[i] {
                    let left_child_ref = Some(Rc::new(RefCell::new(Self::new(value))));
                ...
                if let Some(value) = v[i + 1] {
                    let right_child_ref = Some(Rc::new(RefCell::new(Self::new(value))));
...
mod tests {
    ...
    fn from_bfs_typical_case() {
        let root_ref = TreeNode::from_bfs(vec![Some(1), Some(2), Some(3), Some(4), None, None, Some(5), Some(6), Some(7)]);
        assert_eq!(TreeNode::bfs(root_ref.clone()), vec![1, 2, 3, 4, 5, 6, 7]);
        assert_eq!(TreeNode::dfs(root_ref.clone()), vec![1, 2, 4, 6, 7, 3, 5]);
    }

    #[test]
    #[should_panic(expected = "Invalid array")]
    fn from_bfs_wrong_input_1() {
        TreeNode::from_bfs(vec![None, Some(1), Some(2)]);
    }

    #[test]
    #[should_panic(expected = "Invalid input vector, a None node can't have children")]
    fn from_bfs_wrong_input_2() {
        TreeNode::from_bfs(vec![Some(1), None, None, None]);
    }

    #[test]
    #[should_panic(expected = "Invalid input vector, a None node can't have children")]
    fn from_bfs_wrong_input_3() {
        TreeNode::from_bfs(vec![Some(1), None, Some(2), None, None, None]);
    }

    #[test]
    fn to_bfs_option() {
        let root = TreeNode::from_bfs(vec![Some(1), None, Some(2), Some(3)]);
        assert_eq!(TreeNode::to_bfs_option(root.clone()), vec![Some(1), None, Some(2), Some(3)])
    }
}
