...
pub fn dijkstra<V: Ord + Copy, E: Ord + Copy + Add<Output = E>>(graph: &Graph<V, E>, start: &V) -> BTreeMap<V, Option<(V, E)>> {
    let mut result_tree: BTreeMap<V, Option<(V, E)>> = BTreeMap::new();
    let mut priority_queue: BinaryHeap<Reverse<(E, &V, &V)>> = BinaryHeap::new();

    result_tree.insert(*start, None);

    // Can be put into main dijkstra loop, take it out to avoid unnecessary pattern matching.
    for (new, weight) in &graph[start] {
        result_tree.insert(*new, Some((*start, *weight)));
        priority_queue.push(Reverse((*weight, new, start)));
    }

    while let Some(Reverse((this_dist, new_node, this_node))) = priority_queue.pop() {
        match result_tree[new_node] {
            Some((node, dist)) if node == *this_node && dist == this_dist => {}
            _ => continue,
        }

        // main dijkstra loop
        for (next_node, dist) in &graph[new_node] {
            match result_tree.get(next_node) {
                Some(Some((_, next_dist))) if this_dist + *dist >= *next_dist => {}
                Some(None) => {}
                _ => {
                    result_tree.insert(*next_node, Some((*new_node, this_dist + *dist)));
                    priority_queue.push(Reverse((*dist + this_dist, next_node, new_node)));
                }
            }
        }
    }
